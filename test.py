...
def test(args):
	...
	if args.dataset == 'mvtec':
		...
		test_data = MPDDDataset(root=dataset_dir, transform=preprocess, target_transform=target_transform_b, target_transform_type=target_transform_type, mode='test')
	elif args.dataset == 'mad_sim':
		test_data = MADDataset(root=dataset_dir, transform=preprocess, target_transform=target_transform_b, target_transform_type=target_transform_type, mode='test', datatype='sim')
	elif args.dataset == 'mad_real':
		test_data = MADDataset(root=dataset_dir, transform=preprocess, target_transform=target_transform_b, target_transform_type=target_transform_type, mode='test', datatype='real')
	elif args.dataset == 'real_iad':
		test_data = RealIADDataset_v2(root=dataset_dir, transform=preprocess, target_transform=target_transform_b, target_transform_type=target_transform_type, mode='test')
	else:
	...
	if args.mode == 'few_shot':
		mem_features = memory_fs(args.model, model, obj_list, dataset_dir, save_path, preprocess, target_transform_b, target_transform_type,
							  args.k_shot, few_shot_features, dataset_name, device)

	if args.mode == 'domain_adaption':
		mem_features = memory_da(args.model, model, obj_list, dataset_dir, save_path, preprocess, target_transform_b, target_transform_type,
							 few_shot_features, dataset_name, device)

	# text prompt
	with torch.amp.autocast('cuda', enabled=torch.cuda.is_available()), torch.no_grad():
		if args.dataset == 'mvtec':
	...
	for items in tqdm(test_dataloader, desc="Testing", leave=True):
		...
		with torch.no_grad(), torch.amp.autocast('cuda', enabled=torch.cuda.is_available()):
			...
			# sample
			text_probs = ((image_features @ text_features[0]) * 100.).softmax(dim=-1)
	...
	with ProcessPoolExecutor(max_workers=num_workers) as executor:
		futures = {
			executor.submit(evaluate_one_object, obj, results): obj
			for obj in obj_list
		}

		for future in tqdm(as_completed(futures), total=len(futures), desc="Evaluating objects", leave=True):
			ret = future.result()
